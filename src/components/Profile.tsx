...
// New Firebase imports for Stripe Checkout
...
export const Profile: React.FC = () => {
  ...
  // State for the Stripe loading button
  const [isUpgrading, setIsUpgrading] = useState(false);

  // Frontend mock state for form fields
  ...
  const handleUpgrade = async () => {
    ...
    try {
      // 1. Tell the Stripe Firebase Extension we want to check out
      const docRef = await addDoc(collection(db, "customers", user.id, "checkout_sessions"), {
        price: "price_1THlBr6pqGsIi0D4zZTF2KGt", // <--- ⚠️ PASTE YOUR EXACT STRIPE PRICE ID HERE ⚠️
        success_url: window.location.origin + "/profile",
        cancel_url: window.location.origin + "/profile",
      });

      // 2. Listen to the document. The extension will quickly add a 'url' to it.
      onSnapshot(docRef, (snap) => {
        ...
        if (data?.url) {
          // 3. We got the URL! Redirect the user to the secure Stripe page.
          window.location.assign(data.url);
        }
      });
    } catch (error) {
      console.error("Checkout error:", error);
      alert("Something went wrong loading checkout. Ensure you updated the Price ID in the code!");
      setIsUpgrading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-64 flex-shrink-0">
            <div className="bg-[#161b2b] border border-gray-800 rounded-2xl p-4 flex flex-col gap-2 sticky top-24">
              <TabButton 
              ...
              <button 
                ...
                className="flex items-center gap-3 px-4 py-3 text-red-400 hover:bg-red-500/10 rounded-xl transition-colors text-sm font-medium w-full text-left"
              >
          ...
          <div className="flex-grow">
            <div className="bg-[#161b2b] border border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xl min-h-[500px]">
                ...
                <div className="animate-in fade-in duration-300">
                  ...
                  <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-800">
                    ...
                    <div>
                      ...
                      <button className="mt-2 text-sm text-emerald-500 hover:text-emerald-400 font-medium">
                        Change Avatar
                      </button>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <div className="pt-4 flex justify-end">
                      <button type="submit" className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-emerald-500/20">
                        Save Changes
                      </button>
                    </div>
                  </form>
              ...
              {activeTab === 'subscription' && (
                <div className="animate-in fade-in duration-300">
                  ...
                  <div className="bg-[#0a0f1d] border border-gray-800 rounded-2xl p-6 mb-8">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                      <div>
                        <p className="text-gray-400 text-sm font-bold uppercase tracking-wider mb-1">Current Plan</p>
                        <h3 className="text-2xl font-bold text-white capitalize">
                          {user.subscription} Tier
                        </h3>
                      </div>
                      <div className="bg-[#161b2b] px-4 py-2 rounded-lg border border-gray-800 text-center">
                  ...
                  {/* Upgrade Prompt */}
                  {user.subscription === 'free' && (
                    <div className="border border-emerald-500/30 bg-emerald-500/5 rounded-2xl p-6 relative overflow-hidden group">
                      ...
                      <div className="relative z-10 flex flex-col sm:flex-row justify-between items-center gap-6">
                        ...
                        <div className="text-center w-full sm:w-auto">
                          <p className="text-2xl font-bold text-white mb-3">$19<span className="text-sm text-gray-500">/mo</span></p>
                          <button 
                            onClick={handleUpgrade}
                            disabled={isUpgrading}
                            className="w-full flex justify-center items-center gap-2 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-emerald-500/20"
                          >
                            {isUpgrading && <Loader2 className="w-5 h-5 animate-spin" />}
                            {isUpgrading ? 'Loading Checkout...' : 'Upgrade Now'}
                          </button>
                        </div>
                      </div>
                    </div>
              ...
              {activeTab === 'preferences' && (
                <div className="animate-in fade-in duration-300">
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between p-4 bg-[#0a0f1d] border border-gray-800 rounded-xl">
                      ...
                      <select 
                        ...
                        className="bg-[#161b2b] border border-gray-700 text-white text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block p-2.5 outline-none"
                      >
              ...
              {activeTab === 'security' && (
                <div className="animate-in fade-in duration-300">
                  ...
                  <div className="bg-[#0a0f1d] border border-gray-800 rounded-2xl p-6">
                    ...
                    <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                      ...
                      <div className="pt-2">
                        <button className="bg-gray-800 hover:bg-gray-700 text-white px-6 py-2.5 rounded-xl font-bold transition-colors">
                          Update Password
                        </button>
                      </div>
                    </form>
  ...
  <button
    ...
    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-sm font-medium w-full text-left
      ${active 
        ...
        : 'text-gray-400 hover:bg-[#0a0f1d] hover:text-white border border-transparent'
      }
    `}
  >
    {icon}
...
const InputField = ({ label, name, type = "text", value, onChange, placeholder, disabled = false }: any) => (
  <div>
    ...
    <input 
      ...
      disabled={disabled}
      className={`w-full bg-[#0a0f1d] border border-gray-800 rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
    />
  </div>
);